use image::{codecs::jpeg::JpegEncoder, Pixel, Rgba};
use rand::prelude::*;
use std::fs::File;
...
fn main() {
    ...
    let bgs = vec![palette4[0], palette2[6]];
    let colors = vec![&palette4[2], &palette1[0], &palette1[1], &palette2[0]];
    ...
    for (k, bg) in bgs.iter().enumerate() {
            ...
            for _j in 0..density {
                ...
                for (xx, yy) in shape_coordinates(x, y) {
                    if point_is_visible(xx, yy, image.width(), image.height()) {
                        image
                            .get_pixel_mut(xx, yy)
                            .blend(&color.map_with_alpha(|x| x, |_| alpha));
                    }
                }
            }
        }
        ...
        let mut buffer = File::create(format!("images/day017-{}.jpg", k)).unwrap();
