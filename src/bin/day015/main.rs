...
//    let palettes = vec![palette1, palette2, palette3];
    ...
    for k in 0..4 {
        ...
        for matrix in matrices.iter() {
            ...
            let density: u32 = rng.gen_range(200..250);
            for _j in 0..density {
                let p = scaled_random_point(&mut rng, 1.0, 50.0);
                ...
                for (xx,yy) in shape_coordinates(x,y) {
                    if point_is_visible(xx, yy, image.width(), image.height()) {
                        image.get_pixel_mut(xx, yy).blend(color[k]);
                    }
                }
            }
        }
        image.save(format!("images/day015-{}.png", k)).unwrap();
    }
}
...
fn shape_coordinates(x: u32, y: u32) -> Vec<(u32, u32)> {
    vec![
        ...
        (x-1, y), (x+1, y), (x-2, y), (x+2, y), (x-3,y), (x+3,y),
        (x-1,y+1), (x,y+1), (x+1, y+1),
        (x,y+2), (x, y+3),
