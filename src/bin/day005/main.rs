...
//    let mut rng = Perlin::new().set_seed(2);
    ...
    // image.save("images/day005a.png").unwrap();
    ...
    // image.save("images/day005b.png").unwrap();
    ...
    // image.save("images/day005c.png").unwrap();
}

// fn f1<R: Rng>(x: f64, y: f64, scale: f64, offset: f64, _rng: &mut R, colors: &ListedColorMap) -> Rgb<u8> {
//     let x1 = scale * (x - offset);
//     let y1 = scale * (y - offset);
//     let z = 0.4 * f64::sin(x1 * x1 + y1 * y1) + 0.5; 
...
fn f1<R: Rng>(x: f64, y: f64, scale: f64, offset: f64, rng: &mut R, colors: &ListedColorMap) -> Rgb<u8> {
    ...
    // let z1 = 0.34 * f64::sin(x1 * x1 + y1 * y1) + 0.5; 
    let x2 = scale * (x - offset);
    let y2 = scale * (y - offset);
    let z2 = 0.25 * f64::sin(x2 * x2 + y2 * y2) + 0.5; 
    let output: RGBColor = colors.transform_single(z2);
    Rgb([output.int_r(), output.int_g(), output.int_b()])
}
